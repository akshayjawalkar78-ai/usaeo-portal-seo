...
const statusStyle = {
  closed: { badge: 'bg-destructive/10 text-destructive border-red-200', label: 'Closed' },
  open:   { badge: 'bg-success/10 text-success border-green-200', label: 'Open' },
            ...
            <motion.div key={phase.n} {...fadeUp(i * 0.1)} className={`border rounded-2xl overflow-hidden bg-white transition-all duration-200 ${phase.status === 'closed' ? 'border-border opacity-75' : phase.status === 'open' ? 'border-green-200 hover:shadow-md' : 'border-border hover:border-primary/30 hover:shadow-md'}`}>
              <div className="p-8 md:p-10">
                <div className="flex flex-col md:flex-row md:items-start gap-6">
                  <span className="font-sans text-7xl text-primary/70 leading-none flex-shrink-0">{phase.n}</span>
                  <div className="flex-1">
                    ...
                    <h2 className="font-sans text-3xl text-foreground mb-3">{phase.title}</h2>
